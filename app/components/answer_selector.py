...
def _format_choice(index: int, options: list[str]) -> str:
...
def render_answer_selector(
    ...
    *,
    ...
    """
    Render one vertical radio list: each row is the letter plus full answer text.
    """
    if not options:
        return None
    ...
    picked = st.radio(
        label,
        indices,
        format_func=lambda index: _format_choice(index, options),
        key=widget_key,
        index=None,
